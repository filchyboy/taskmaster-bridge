...
export class JiraClient {
     ...
     */
    async findSubtaskType(projectKey) {
        if (this.subtaskType) {
        ...
        const subNameType = issueTypes.find(type => type.name.toLowerCase().includes('sub') ||
    ...
    async countIssues(projectKey, includeSubtasks = false) {
    ...
    async fetchIssues(projectKey, startAt = 0, maxResults = 100, includeSubtasks = false) {
        const jql = includeSubtasks
